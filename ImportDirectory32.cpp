...
ImageSectionHeader ImportDirectory32::locate(DWORD VA) {
...
DWORD ImportDirectory32::resolve(DWORD VA, ImageSectionHeader Section) {
	return ((VA - Section.VirtualAddress) + Section.PointerToRawData);
}

ImportDirectory32::ImportDirectory32(FILE* PEFile, NTHeaders32* _NTHeaders, SectionHeaders* _SectionHeaders) {
...
void ImportDirectory32::PrintImportDirectoryInfo(FILE* PEFile) {
	...
	for (const ImageImportDescriptor& CurImportDescriptor : ImageImportDescriptorVec) {
		...
		while (true) {
			...
			fseek(PEFile, (ILTAddr + (entrycounter * sizeof(DWORD))), SEEK_SET);
			fread(&entry, sizeof(ILTEntry32), 1, PEFile);
